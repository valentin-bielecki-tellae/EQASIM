...
def configure(context):
    context.stage("data.hts.emc_85.cleaned")

def execute(context):
    df_households, df_persons, df_trips = context.stage("data.hts.emc_85.cleaned")

    # EMC defines multiple weights. For comparison with EGT we keep them in the
    # data set for the previous stages. In this one we override the weight,
    # because the initial person_weight is valid for all persons. However, EMC
    ...
    # initially they fit EGT and census well). EMC already defines weights which
