...
def configure(context):
    context.stage("data.hts.emc_85.raw")
...
MODES_MAP = {
    ...
    "bike": [10, 11, 17, 12, 18, 93, 19],
    "walk": [1, 2] # Actually, 2 is not really explained, but we assume it is walk
}

def execute(context):
    df_households, df_persons, df_trips = context.stage("data.hts.emc_85.raw")

    # Merge departement into households
    df_households["departement_id"] = "85"

    # Transform original IDs to integer (they are hierarchichal)
    df_households["emc_household_id"] = (df_households["ECH"] + df_households["MP2"]).astype(int)
    df_persons["emc_person_id"] = df_persons["PER"].astype(int)
    df_persons["emc_household_id"] = (df_persons["ECH"] + df_persons["PP2"]).astype(int)
    df_trips["emc_person_id"] = df_trips["PER"].astype(int)
    df_trips["emc_household_id"] = (df_trips["ECH"] + df_trips["DP2"]).astype(int)
    df_trips["emc_trip_id"] = df_trips["NDEP"].astype(int)
    ...
    df_persons = pd.merge(
        df_persons, df_households[["emc_household_id", "household_id", "departement_id"]],
        on = ["emc_household_id"]
    ).sort_values(by = ["household_id", "emc_person_id"])
    df_persons["person_id"] = np.arange(len(df_persons))

    df_trips = pd.merge(
        df_trips, df_persons[["emc_person_id", "emc_household_id", "person_id", "household_id"]],
        on = ["emc_person_id", "emc_household_id"]
    ).sort_values(by = ["household_id", "person_id", "emc_trip_id"])
    ...
    # Clean departement
    df_trips["origin_departement_id"] = "85"
    df_trips["destination_departement_id"] = "85"
    ...
    # Has subscription (not availabile in emc 44)
    ...
    # Trip times
    df_trips["departure_time"] = 3600.0 * df_trips["D4"].divmod(100)[0] # hour
    df_trips["departure_time"] += 60.0 * df_trips["D4"].divmod(100)[1] # minute

    df_trips["arrival_time"] = 3600.0 * df_trips["D8"].divmod(100)[0] # hour
    df_trips["arrival_time"] += 60.0 * df_trips["D8"].divmod(100)[1] # minute
