...
def process(context, purpose, random, df_persons, df_od, df_locations):
    ...
    with context.progress(
        label="Sampling %s municipalities" % purpose, total=len(df_demand)
    ) as progress:
...
def execute(context):
    ...
    df_work = process(context, "work", random, df_persons, df_work_od, df_locations)
    ...
    for prefix, education_type in EDUCATION_MAPPING.items():
        df_education.append(
            process(
                ...
                random,
                df_persons[
                    df_persons["age"].between(
                        education_type["min_age"],
                        education_type["max_age"],
                    )
                ],
                df_education_od,
                df_locations[
                    df_locations["TYPEQU"].str.startswith(education_type["type_edu"])
                ],
            )
        )
